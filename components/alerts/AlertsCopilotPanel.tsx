...
export function AlertsCopilotPanel({
  ...
  const [messages, setMessages] = useState<Msg[]>([
    {
      role: "assistant",
      content: pt
        ? "Posso sugerir alertas melhores, remover ruído, e deixar o SignalCore a trabalhar por ti."
        : "I can suggest better alerts, remove noise, and keep SignalCore working for you.",
    },
  ]);
    ...
    () =>
      pt
        ? [
            ...
            "Quero um briefing semanal automático.",
          ]
        : [
  ...
  async function send(text: string) {
    ...
    try {
      // lightweight: we don't need a full LLM now
      // We'll generate 1-2 example alerts automatically as "smart suggestions".
      // (Later we route into /api/chat with context.)
      ...
      if (t.toLowerCase().includes("weekly") || t.toLowerCase().includes("semanal")) {
        await alertsStore.create({
          type: "advisor",
          title: pt ? "Weekly briefing enabled" : "Weekly briefing enabled",
          message: pt
            ? "Todas as semanas vou resumir o teu plano, drift, e o melhor próximo passo."
            : "Every week I’ll summarize your plan, drift, and the next best action.",
          severity: "success",
          dedupe_key: "alerts_weekly_briefing_v1",
          action: { label: pt ? "Abrir Daily" : "Open Daily", href: "/app/daily" },
        });
        ...
        setMessages((m) => [
          ...m,
          {
            role: "assistant",
            content: pt
              ? "Feito. Ativei o briefing semanal (sem spam)."
              : "Done. Weekly briefing is enabled (no spam).",
          },
        ]);
      } else {
        await alertsStore.create({
          type: "risk",
          title: pt ? "Risk drift watch enabled" : "Risk drift watch enabled",
          message: pt
        ...
        setMessages((m) => [
          ...m,
          {
            role: "assistant",
            content: pt
              ? "Ativei um alerta institucional de drift de risco. Se quiseres, adiciono breach + concentração."
      ...
      setMessages((m) => [
        ...m,
        {
          role: "assistant",
          content: pt
            ? "Erro ao criar alertas. Tenta novamente."
            : "Failed to create alerts. Try again.",
        },
      ]);
    } finally {
      setBusy(false);
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-ink-600">
            {pt
              ? "O Copilot cria alertas inteligentes sem te bombardear."
              : "Copilot creates smart alerts without spamming you."}
          </div>
        </div>
