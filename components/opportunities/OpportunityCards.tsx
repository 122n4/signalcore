...
export default function OpportunityCards({
  ...
  async function sendToExecution(opp: Opportunity) {
    ...
    // 1) Always store locally so UX never fails
    executionClientStore.push(item);

    // 2) Try server insert (Supabase)
    const ok = await enqueueExecutionServer(item);
    setSendingId(null);

    setToast(ok ? "Sent to Execution Queue ✅" : "Saved locally (server sync failed) ⚠️");
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm font-semibold text-ink-900">
              {money(suggestedAmount, base)} per action
            </div>
            <div className="mt-1 text-xs text-ink-600">
              Based on your current cash. Keep moves disciplined and repeatable.
            </div>
          </div>
          ...
          <div key={opp.id} className="rounded-2xl border border-border-soft bg-white p-4 shadow-soft">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="mt-1 text-xs text-ink-600">
                  {opp.symbol} • {opp.vehicle} • Risk:{" "}
                  <span className="font-semibold">{opp.riskLabel}</span> • Horizon:{" "}
                  <span className="font-semibold">{opp.horizon}</span>
                </div>
              </div>

              <div className="flex flex-col items-end gap-1">
                <div className="rounded-full border border-border-soft bg-white px-2 py-0.5 text-[11px] text-ink-700">
              ...
              <div className="text-xs text-ink-600">
                ...
                {pro && (
                  <span className="ml-2 text-[11px] text-ink-500">
                    • cap {opp.maxSizePct}% • bucket {opp.bucketHint}
                  </span>
                )}
              </div>
      ...
      <div className="rounded-2xl border border-border-soft bg-white p-4 shadow-soft">
        <div className="text-xs font-semibold text-ink-700">How this makes you money (without hype)</div>
        <ul className="mt-2 list-disc space-y-1 pl-4 text-xs text-ink-600">
          <li>We rank opportunities by plan fit + regime alignment + estimated odds impact.</li>
          <li>We cap sizing to protect you from drawdowns and concentration traps.</li>
          <li>You execute inside your broker — SignalCore stays legal and institution-grade.</li>
        </ul>
      </div>
    </div>
  );
