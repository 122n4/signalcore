...
export function CopilotToolbelt(props: {
  context: string;              // ex: "risk"
  state?: Record<string, any>;  // snapshot (portfolio/plan/engine)
  title?: string;              // ex: "Copilot"
  ...
  async function run(q: QuickAction) {
    ...
    try {
      ...
      journal.log({
        ...
        meta: { context, question: q.question, assumptions: data.assumptions ?? [], confidence: data.confidence ?? null },
      });
