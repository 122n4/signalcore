...
export type InvestingEnginePhase4CScopedScanV1 = Readonly<{
...
type ScannerDependencies = Readonly<{
  ...
  scope?: InvestingEnginePhase4CScopedScanV1;
}>;
...
export class InvestingEnginePhase4CIntegrityScanner {
  ...
  async scan(): Promise<InvestingEnginePhase4CIntegrityReport> {
    ...
    const remainingMs = () => this.dependencies.scope?.remainingMs?.()
      ?? Math.max(
        0,
        Math.ceil(
          (this.dependencies.scope?.timeoutMs ?? Number.MAX_SAFE_INTEGER)
          - (globalThis.performance.now() - startedAt),
        ),
      );
    const assertBudget = () => {
      if (this.dependencies.scope && remainingMs() === 0) {
        throw new Error("investing_phase4c_scanner_timeout");
      }
    };
    try {
      ...
      if (scope) {
        if (
          !Number.isSafeInteger(scope.maxRuns)
          || scope.maxRuns < 1
          || scope.maxRuns > 100
          ...
          throw new Error("investing_phase4c_scanner_budget_invalid");
        }
        await client.query("set local role authenticated");
        await client.query(
          "select set_config('request.jwt.claims', $1, true)",
          [JSON.stringify({ sub: scope.authenticatedUserId })],
        );
        const snapshotStatementTimeoutMs = remainingMs();
        if (snapshotStatementTimeoutMs === 0) {
          throw new Error("investing_phase4c_scanner_timeout");
        }
        await client.query(
          "select set_config('statement_timeout', $1, true)",
          [`${snapshotStatementTimeoutMs}ms`],
        );
        ...
        if ((count.rows[0]?.count ?? 0) > scope.maxRuns) {
          throw new Error("investing_phase4c_scanner_budget_exceeded");
        }
      }
      ...
      if (scope) {
        const crossScope = [
          ...
          row.ownerId !== scope.ownerId || row.accountId !== scope.accountId);
        if (crossScope) {
          throw new Error("investing_phase4c_scanner_scope_mismatch");
        }
      }
