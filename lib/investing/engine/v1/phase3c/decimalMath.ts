...
function powerOfTen(exponent: number) {
  if (!Number.isSafeInteger(exponent) || exponent < 0) {
    throw new Error("investing_decimal_scale_invalid");
  }
...
export function decimalSubtract(left: CanonicalDecimal, right: CanonicalDecimal): CanonicalDecimal {
  const [leftCoefficient, rightCoefficient, scale] = align(parse(left), parse(right));
  return fromParts(leftCoefficient - rightCoefficient, scale);
}
...
export function decimalDivide(
  ...
  if (bottom.coefficient === BigInt(0)) throw new Error("investing_decimal_division_by_zero");
  if (!Number.isSafeInteger(precision) || precision < 0 || precision > 36) {
    throw new Error("investing_decimal_precision_invalid");
  }
...
export function decimalMax(left: CanonicalDecimal, right: CanonicalDecimal): CanonicalDecimal {
...
export function decimalAbs(value: CanonicalDecimal): CanonicalDecimal {
  return decimalCompare(value, DECIMAL_ZERO) < 0 ? decimalSubtract(DECIMAL_ZERO, value) : value;
}
...
export function decimalIsNegative(value: CanonicalDecimal) {
