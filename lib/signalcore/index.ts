...
export type EngineV2SuggestedAllocationRow = {
  bucket: string;
  weight: number;
};

export type EngineV2Action = {
  title: string;
  detail: string;
...
export function runEngineV2(input: {
  ...
  // If you already have a real engine v2 function, plug it here.
  // This wrapper keeps the Advisor stable.
  ...
  const drift =
    overall < 60 ? "High" : overall < 75 ? "Medium" : "Low";
  ...
  const guardrails = [
    ...
    "Max crypto: 15%",
    "Min cash buffer: 3%",
    "No leverage until coherence > 80",
  ];

  const topActions: EngineV2Action[] = [
    {
      title: "Fix your biggest coherence driver",
      detail: "Your returns are limited more by decision quality than by picking winners. Fix structure first.",
      severity: overall < 65 ? "high" : "medium",
    },
    {
      title: "Only rebalance inside your bands",
      detail: "Avoid emotional reallocations. Use a disciplined cadence and guardrails.",
      severity: "low",
    },
  ];
