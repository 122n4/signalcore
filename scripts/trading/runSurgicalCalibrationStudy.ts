...
const OUTPUT_PATH = path.resolve(
  "artifacts/trading-backtests/trading-surgical-calibration-study-local-2020-2025-live_playbook_market_session_calibrated.json",
);

const SCENARIOS: ScenarioDefinition[] = [
  {
    id: "xauusd_late_us_blocked",
    description: "Block XAUUSD in late US where the session is slightly negative and drags a strong market.",
    rules: [
      {
        instrument: "XAUUSD",
        sessions: ["late_us"],
        reason: "Surgical calibration blocked XAUUSD during late US.",
      },
    ],
  },
  {
    id: "eurusd_residual_weak_sessions_blocked",
    description: "Block EURUSD residual weak sessions and keep only the surviving positive Asia flow block.",
    rules: [
      {
        instrument: "EURUSD",
        sessions: ["london_open", "late_us", "london_ny_overlap"],
        reason: "Surgical calibration blocked residual weak EURUSD sessions.",
      },
    ],
  },
  {
    id: "nas100_late_us_blocked",
    description: "Block NAS100 late US where expectancy is near flat and quality is weak.",
    rules: [
      {
        instrument: "NAS100",
        sessions: ["late_us"],
        reason: "Surgical calibration blocked NAS100 during late US.",
      },
    ],
  },
  {
    id: "us500_pre_market_blocked",
    description: "Block US500 pre-market where profit factor is barely above 1 and drawdown is concentrated.",
    rules: [
      {
        instrument: "US500",
        sessions: ["pre_market"],
        reason: "Surgical calibration blocked US500 during pre-market.",
      },
    ],
  },
  {
    id: "refined_surgical_stack",
    description: "Stack the refined surgical blocks together to test for a compounding quality effect.",
    rules: [
      {
        instrument: "XAUUSD",
        sessions: ["late_us"],
...
function buildScenarioBacktest(rules: TradingBacktestMarketSessionRule[]) {
  return rules.length > 0
    ? {
        captureSteps: false,
        marketSessionOverrides: {
          blockedTradeValidContexts: rules,
        },
      }
    : {
        captureSteps: false,
...
function collectTradesFromComparative(
  ...
  return {
    ...
    evaluatedBars: selectedMarkets.reduce(
...
function buildMergedSummary(args: {
  ...
  const scenarioChanged = collectTradesFromComparative(
    args.scenarioAffected,
    args.affectedInstruments,
  );
...
async function ensureBaselineFullCrisis(args: {
  instruments: string[];
  timeframes: string[];
}): Promise<TradingBacktestComparativeReport> {
  try {
    return await readJsonFile<TradingBacktestComparativeReport>(BASELINE_FULL_CRISIS_PATH);
  } catch {
    const report = await runTradingHistoricalComparativeSweep({
      ...
      backtest: buildScenarioBacktest([]),
    });

    await writeTradingHistoricalComparativeReport({
      report,
      outputPath: BASELINE_FULL_CRISIS_PATH,
    });
...
async function main() {
  ...
  if (requestedScenarioIds.size > 0 && scenariosToRun.length === 0) {
    throw new Error(
      `No surgical calibration scenarios matched: ${Array.from(requestedScenarioIds).join(", ")}`,
    );
  }
  ...
  const baselineFullCrisis = await ensureBaselineFullCrisis({
    instruments: baselineFullYearly.request.instruments,
  ...
  for (const scenario of scenariosToRun) {
    ...
    const { report: affectedYearlyReport, outputPath: affectedYearlyPath } =
      await runAffectedComparative({
        periods: YEARS,
        rules: scenario.rules,
        instruments: affectedInstruments,
        timeframes: baselineFullYearly.request.timeframes,
        outputName: `trading-surgical-affected-yearly-${scenario.id}.json`,
      });

    const { report: affectedCrisisReport, outputPath: affectedCrisisPath } =
      await runAffectedComparative({
        periods: CRISIS_PERIODS,
        rules: scenario.rules,
        instruments: affectedInstruments,
        timeframes: baselineFullYearly.request.timeframes,
        outputName: `trading-surgical-affected-crisis-${scenario.id}.json`,
      });
    ...
    const walkForwardSummary = computeSummaryFromComparative(
      affectedYearlyReport,
    ...
    const aggregateImproved =
      aggregateSummary.totalTrades >= baselineAggregateSummary.totalTrades &&
    ...
    const walkForwardNotWorse =
      walkForwardSummary.totalTrades >= baselineWalkForwardSummary.totalTrades &&
  ...
  const report = {
    ...
    keepableScenarios: scenarioResults.filter((scenario) => scenario.gates.keep).map((scenario) => ({
      id: scenario.id,
      description: scenario.description,
