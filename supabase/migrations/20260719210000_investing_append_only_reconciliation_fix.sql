-- A reconciliation run is immutable. Compute every break first, then insert
-- the final run and its append-only items; never update the run after insert.
...
declare
  ...
  v_fill_completion_break integer := 0;
  v_breaks integer := 0;
...
begin
  ...
  if not found then
    raise exception 'investing_order_not_found_or_forbidden';
  end if;
  if v_order.status not in ('reconciling', 'reconciled') then
    raise exception 'investing_order_not_reconciling';
  end if;
  ...
  select coalesce(sum(quantity), 0) into v_fill_qty
  ...
  from (
    ...
    where t.account_id = v_order.account_id
      and t.source_type = 'fill'
      and t.source_id in (
        select fill_id from public.investing_fills where order_id = v_order.id
      )
    group by t.id
    having round(sum(case when e.side = 'debit' then e.amount else -e.amount end), 8) <> 0
  ) broken;

  if v_order.status <> 'reconciled'
     and v_order.cumulative_filled_quantity <> v_order.quantity then
    v_fill_completion_break := 1;
  end if;

  select count(*) into v_cash_breaks
  from public.investing_cash_balances b
  where b.account_id = v_order.account_id
    and (
      round(b.available_amount, 8) <> round(coalesce((
        ...
        where e.account_id = b.account_id
          and e.account_code = 'cash'
          and e.currency = b.currency
      ), 0), 8)
      or round(b.available_amount, 8) <> round(b.settled_amount, 8)
    );

  select count(*) into v_reserved_breaks
  from public.investing_cash_balances b
  where b.account_id = v_order.account_id
    and round(b.reserved_amount, 8) <> round(coalesce((
      select sum(o.reserved_cash_amount)
  ...
  select count(*) into v_position_breaks
  from public.investing_positions p
  where p.account_id = v_order.account_id
    and (
      ...
      or round(p.reserved_quantity, 12) <> round(coalesce((
        select sum(o.reserved_position_quantity)
        from public.investing_orders o
  ...
  select count(*) into v_fee_breaks
  from public.investing_fills f
  where f.order_id = v_order.id
    and (
      round(f.fee_amount, 8) <> round(coalesce((
        select sum(x.amount) from public.investing_fees x
        where x.fill_id = f.id and x.fee_type <> 'tax'
      ), 0), 8)
      or round(f.tax_amount, 8) <> round(coalesce((
        select sum(x.amount) from public.investing_fees x
        where x.fill_id = f.id and x.fee_type = 'tax'
      ), 0), 8)
    );

  select count(*) into v_missing_fill_ledgers
  from public.investing_fills f
  where f.order_id = v_order.id
    and not exists (
      ...
      where t.account_id = v_order.account_id
        and t.source_type = 'fill'
        and t.source_id = f.fill_id
    );

  select count(*) into v_queue_breaks
  from public.investing_execution_queue q
  where q.id = v_order.queue_id
    and q.operational_state <> v_order.status;

  v_breaks :=
    case when v_fill_qty <> v_order.cumulative_filled_quantity then 1 else 0 end
    + v_unbalanced
    + v_fill_completion_break
    + v_cash_breaks
    + v_reserved_breaks
    + v_position_breaks
    + v_fee_breaks
    + v_missing_fill_ledgers
    + v_queue_breaks;

  insert into public.investing_reconciliation_runs(
    user_id, portfolio_id, account_id, status, score,
    correlation_id, environment, completed_at
  ) values (
    v_order.user_id, v_order.portfolio_id, v_order.account_id,
    case when v_breaks = 0 then 'passed' else 'failed' end,
    case when v_breaks = 0 then 100 else 0 end,
    p_correlation_id, 'paper', now()
  ...
  if v_fill_completion_break > 0 then
    insert into public.investing_reconciliation_items(run_id,item_type,severity,expected,observed,difference)
  ...
  set status = case when v_breaks = 0 then 'reconciled' else 'reconciliation_failed' end,
      version = version + 1,
      updated_at = now()
  where id = v_order.id;

  update public.investing_execution_queue
  set operational_state = case when v_breaks = 0 then 'reconciled' else 'reconciliation_failed' end,
      version = version + 1,
      updated_at = now()
  where id = v_order.queue_id;

  insert into public.investing_execution_events(
    user_id,portfolio_id,account_id,order_id,event_type,severity,
    environment,correlation_id,engine_version,payload
  ) values (
    v_order.user_id,v_order.portfolio_id,v_order.account_id,v_order.id,
    'paper_reconciliation_completed',case when v_breaks=0 then 'info' else 'error' end,
    'paper',p_correlation_id,'investing_v2',jsonb_build_object('run_id',v_run_id,'breaks',v_breaks)
  );
...
revoke all on function public.investing_reconcile_paper_order_v2(text,uuid,text)
  from public, anon, authenticated;
grant execute on function public.investing_reconcile_paper_order_v2(text,uuid,text)
  to service_role;
