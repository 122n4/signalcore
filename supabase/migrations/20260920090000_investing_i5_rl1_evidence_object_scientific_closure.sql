...
create policy research_execution_evidence_select on investing.research_evidence_objects_scientific_identities for select to investing_app
  using (operation = 'RESEARCH_EXECUTION_RUN_V1' and capability = 'RESEARCH_EXECUTE' and operation_scope = 'TENANT_SCOPE' and source_context = 'PURE_RESEARCH' and account_id is null and current_setting('syntrake.investing.operation', true) = 'RESEARCH_EXECUTION_RUN_V1' and current_setting('syntrake.investing.capability', true) = 'RESEARCH_EXECUTE' and current_setting('syntrake.investing.operation_scope', true) = 'TENANT_SCOPE' and current_setting('syntrake.investing.source_context', true) = 'PURE_RESEARCH' and nullif(current_setting('syntrake.investing.account_id', true), '') is null and nullif(current_setting('syntrake.investing.account_access_id', true), '') is null and tenant_id::text = current_setting('syntrake.investing.tenant_id', true) and principal_id::text = current_setting('syntrake.investing.principal_id', true) and tenant_membership_id::text = current_setting('syntrake.investing.tenant_membership_id', true));
