...
describe("Investing branch consolidation proof", () => {
  ...
  it("preserves historical baseline admission docs while candidate runtime promotes Experiment hashing", () => {
    ...
    expect(hashDomains).toContain("`SYNTRAKE:EXPERIMENT:V1` | `DECLARED_BUT_HASHING_DISABLED`");
    ...
    expect(barrel).toContain("hashExperimentV1");
  });
});
