...
const connectionString = process.env.PG17_REHEARSAL_URL ?? process.env.PG17_RECONCILIATION_URL ?? "";
const demoProjectRef = "local-pg17";
const primarySubject = "user_pg17_i0_i5_primary";
const secondarySubject = "user_pg17_i0_i5_secondary";
...
async function seedInitialPaperCashFunding(context: AuthorizedInvestingContext, amount: string) {
  ...
  const correlationId = "corr-i0-i5-funding-0001";
  const idempotencyKey = "idem-i0-i5-funding-0001";
  ...
  try {
    ...
    await adminClient.query(
      `insert into investing.ledger_transactions (
        ...
        'USER_DECLARED_PAPER_CAPITAL','I0_I5_PG17_DEMO_FUNDING','SIMULATED','NOT_APPLICABLE','DEMO'
      )`,
...
(connectionString ? describe : describe.skip)("Investing Genesis I0 -> I5 cumulative PostgreSQL 17 rehearsal", () => {
  it("replays I0 -> I4 -> I5 in one database and preserves I3/I4 contracts through RL-1", async () => {
    ...
    const bootstrap = await bootstrapInitialPersonalInvestingAccount({
        idempotencyKey: "idem-i0-i5-bootstrap-0001",
        correlationId: "corr-i0-i5-bootstrap-0001",
      baseCurrency: "EUR",
    });
    ...
    const authorityBeforeI3 = await resolveAuthorizedInvestingAccountContext({
      ...
      correlationId: "corr-i0-i5-authority-0001",
    });
    ...
    await adminClient.query(
      ...
      ) values ($1,'SIMPLE_CASH_SECURITY','EUR','ACTIVE','SYNTHETIC_I3_REHEARSAL','I0_I5_PG17_INSTRUMENT','DEMO')`,
      [instrumentId],
    );

    const authority = await resolveAuthorizedInvestingAccountContext({
      accountId: bootstrap.accountId,
      correlationId: "corr-i0-i5-authority-0002",
    });
    ...
    const buyInput = {
      ...
      idempotencyKey: "idem-i0-i5-buy-00000001",
      correlationId: "corr-i0-i5-buy-00000001",
      instrumentId,
      side: "BUY" as const,
      quantity: "10",
      unitPrice: "100",
      ...
      effectiveAt,
      sourceSequence: "1",
      sourceReference: "I0_I5_PG17_BUY_1",
    };
    ...
    const sellResults = await Promise.all([
      accountSyntheticI3Fill({
        ...concurrentSellBase,
        idempotencyKey: "idem-i0-i5-sell-a-0001",
        correlationId: "corr-i0-i5-sell-a-0001",
        sourceSequence: "2",
        sourceReference: "I0_I5_PG17_SELL_A",
      }, i3Env),
      accountSyntheticI3Fill({
        ...concurrentSellBase,
        idempotencyKey: "idem-i0-i5-sell-b-0001",
        correlationId: "corr-i0-i5-sell-b-0001",
        sourceSequence: "3",
        sourceReference: "I0_I5_PG17_SELL_B",
      }, i3Env),
    ]);
    ...
    const initInput = {
      accountId: bootstrap.accountId,
      idempotencyKey: "idem-i0-i5-plan-init-0001",
      correlationId: "corr-i0-i5-plan-init-0001",
    ...
    const revisionCalls = [
      {
        ...revisionBase,
        idempotencyKey: "idem-i0-i5-plan-r2-a-01",
        correlationId: "corr-i0-i5-plan-r2-a-01",
      },
      {
        ...revisionBase,
        idempotencyKey: "idem-i0-i5-plan-r2-b-01",
        correlationId: "corr-i0-i5-plan-r2-b-01",
      },
    ] as const;
    ...
    const secondaryBootstrap = await bootstrapInitialPersonalInvestingAccount({
        idempotencyKey: "idem-i0-i5-bootstrap-0002",
        correlationId: "corr-i0-i5-bootstrap-0002",
      baseCurrency: "EUR",
    });
    ...
    const crossAccount = await createAndActivateInvestingPlanRevisionForAccountV1({
      ...
      idempotencyKey: "idem-i0-i5-cross-account-01",
      correlationId: "corr-i0-i5-cross-account-01",
      content: allNotSuppliedContent,
    });
    ...
    const denialContextResult = await resolveAuthorizedInvestingAccountContext({
      accountId: bootstrap.accountId,
      correlationId: "corr-i0-i5-denial-context",
    });
    ...
    const writerDenial = await initializePlanV1({
      ...
      idempotencyKey: "idem-i0-i5-plan-denial-01",
      correlationId: "corr-i0-i5-plan-denial-01",
      content: allNotSuppliedContent,
    });
    ...
    for (const migration of i5Migrations) {
    ...
    let serviceRoleDeniedAfterI5 = false;
    await adminClient.query("begin");
    try {
      await adminClient.query("set local role service_role");
      await adminClient.query("select research_investigation_id from investing.research_investigations limit 1");
    } catch (error) {
      serviceRoleDeniedAfterI5 = (error as { code?: string }).code === "42501";
    } finally {
      await adminClient.query("rollback");
    }
    expect(serviceRoleDeniedAfterI5).toBe(true);

    const authorityAfterI5 = await resolveAuthorizedInvestingAccountContext({
      accountId: bootstrap.accountId,
      correlationId: "corr-i0-i5-authority-after-i5",
    });
    expect(authorityAfterI5.ok).toBe(true);
    if (authorityAfterI5.ok !== true) throw new Error("I3 authority after I5 failed");

    const postI5Buy = await accountSyntheticI3Fill({
      ...buyInput,
      authorizedContext: authorityAfterI5.context,
      idempotencyKey: "idem-i0-i5-buy-after-i5-01",
      correlationId: "corr-i0-i5-buy-after-i5-01",
      sourceSequence: "4",
      sourceReference: "I0_I5_PG17_BUY_AFTER_I5",
    }, i3Env);
    expect(postI5Buy.ok).toBe(true);

    const postI5PlanRevision = await createAndActivateInvestingPlanRevisionForAccountV1({
      accountId: bootstrap.accountId,
      planRootId: initialPlan.planRootId,
      expectedActiveRevisionId: revisionWinner.planRevisionId,
      expectedActiveVersion: "2",
      idempotencyKey: "idem-i0-i5-plan-r3-after-i5",
      correlationId: "corr-i0-i5-plan-r3-after-i5",
      content: allNotSuppliedContent,
    });
    expect(postI5PlanRevision.ok).toBe(true);
    if (postI5PlanRevision.ok !== true) throw new Error("I4 Plan revision after I5 failed");
    expect(postI5PlanRevision.activeVersion).toBe("3");
  }, 120_000);
});
