...
describe("Investing Genesis I3-A accounting foundations source candidate", () => {
  ...
  it("pins bounded decimal persistence and a fail-closed no-implicit-rounding V1", () => {
    ...
    const quantityFunction = normalized.indexOf("create function investing.i3_is_canonical_quantity_v1");
    const quantityRevoke = normalized.indexOf("revoke all on function investing.i3_is_canonical_quantity_v1(text)");
    const nullCheck = normalized.indexOf("if investing.i3_is_canonical_quantity_v1(null)");
    const resetRole = normalized.indexOf("reset role", nullCheck);
    const finalPostcondition = normalized.indexOf("do $$ declare v_inventory text[]", resetRole);
    expect(quantityFunction).toBeGreaterThanOrEqual(0);
    expect(quantityRevoke).toBeGreaterThan(quantityFunction);
    expect(nullCheck).toBeGreaterThan(quantityRevoke);
    expect(normalized).toContain("i3-a validator integrity violation: null decimal text must fail closed");
    expect(resetRole).toBeGreaterThan(nullCheck);
    expect(finalPostcondition).toBeGreaterThan(resetRole);
    expect(normalized.slice(finalPostcondition)).not.toContain("i3_is_canonical_quantity_v1(null)");
    expect(normalized.slice(finalPostcondition)).not.toContain("i3_is_canonical_positive_money_v1(null)");
    expect(normalized.slice(finalPostcondition)).not.toContain("i3_is_canonical_nonnegative_money_v1(null)");
