...
const runtimeLockMigrationPath = path.join(
  repoRoot,
  "supabase",
  "migrations",
  "20260911110000_investing_i5_research_runtime_lock_contract_repair.sql",
);
const a3MigrationPath = path.join(repoRoot, "supabase", "migrations", "20260912050000_investing_i5_a3_research_material_revisions.sql");
...
describe("Investing Genesis I5-A4 Research Spec persistence", () => {
  ...
  it("uses PostgreSQL catalog privilege introspection that is stable under INHERIT FALSE roles", () => {
    ...
    expect(prestate).toContain("if current_user <> 'postgres' then");
    expect(sql).not.toContain("information_schema.column_privileges");
    expect(sql).not.toContain("information_schema.role_table_grants");
    expect(sql).toContain("pg_catalog.has_column_privilege(");
    expect(sql).toContain("pg_catalog.has_table_privilege(");
    expect(sql).toContain("'investing_app'");
    expect(sql).not.toContain("set role investing_owner");
    ...
    for (const column of [
      ...
      expect(prestate).toContain(`('${column}')`);
    }
    expect(prestate).not.toContain("('active_spec_revision_id')");
    expect(prestate).not.toContain("('active_experiment_id')");
    ...
    for (const column of [
      ...
      expect(poststate).toContain(`('${column}')`);
    }
    expect(poststate).not.toContain("('active_experiment_id')");

    for (const privilege of ["'select'", "'insert'", "'update'", "'delete'", "'truncate'", "'references'", "'trigger'"]) {
      expect(sql).toContain(privilege);
    }
    expect(sql).toContain("pointer table select/insert grants are not exact");
    expect(sql).toContain("research_spec_revisions grants are not exact");
    expect(sql).toContain("unexpected blocked role grant on spec revisions");
  });

  it("pins the repaired canonical predecessor blobs and A3 policy name before A4 rebuild", () => {
    const runtimeLock = read(runtimeLockMigrationPath);
    const a3 = read(a3MigrationPath);
    const a4 = normalize(read(migrationPath));
    const oldA3Policy = "tenant_memberships_i5_a3_material_revision_account_authority_read";
    ...
    expect(a4).toContain("pg_catalog.pg_get_expr(d.adbin, d.adrelid) = 'transaction_timestamp()'");
    expect(a4).toContain("pg_catalog.has_column_privilege('investing_app', c.oid, a.attname, 'update')");
    expect(a4).toContain("not pg_catalog.has_table_privilege('investing_app', c.oid, 'update')");
  });
  ...
  it("keeps A4 policy identifiers within PostgreSQL's 63-byte limit without truncation collisions", () => {
