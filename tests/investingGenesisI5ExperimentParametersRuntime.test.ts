...
describe("Investing Genesis I5 ExperimentParameters scientific identity runtime", () => {
  ...
  it("does not mutate inputs and introduces no server, DB, Experiment, Paper, Trading, Core or execution authority", () => {
    ...
    expect(contract).toContain("Persistence evolution is future work");
    ...
    expect(state).toContain("ExperimentParameters persistence is NOT yet implemented");
    expect(state).toContain("Current VARIANT persistence still cannot distinguish same-parent variants");
    ...
    expect(state).not.toContain("SCIENTIFIC EXPERIMENT IDENTITY = CURRENT_ACCEPTED");
  });
});
